            ...
            if (user_data) {
               bcrypt.compare(data.password,user_data.password,function (err,check) {
                   if (check) {
                    ...
                    res.status(403).send({message:'Correo o contraseña incorrecta'});
                   }
               });
            }else{
...
function editar(req,res){
    ...
    if(data.password){
        bcrypt.hash(data.password,null,null,function(err,hash){
            if(hash){
                User.findByIdAndUpdate(id,{nombres: data.nombres,apellidos: data.apellidos,  email: data.email,password: hash, dni : data.dni, telefono : data.telefono, role:  data.role},(err,user_edit)=>{
                    if(user_edit){
        ...
        User.findByIdAndUpdate(id,{nombres: data.nombres,apellidos: data.apellidos,  email: data.email, dni : data.dni, telefono : data.telefono, role:  data.role},(err,user_edit)=>{
            if(user_edit){
                res.status(200).send({user:user_edit});
            }else{
                res.status(500).send({message: 'El usuario no se pudo editar'});
